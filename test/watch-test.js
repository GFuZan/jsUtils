...
watch(data, ["x"], (newVal, oldVal, key) => {
  console.log(
    `监听未定义值: X = ${data["x"]}, newVal = ${newVal}, oldVal = ${oldVal}, key = ${key}`
  );
});
...
watch(
  ...
  (newVal, oldVal, key) => {
    console.log(
      `监听A: A = ${data["a"]}, newVal = ${newVal}, oldVal = ${oldVal}, key = ${key}`
    );
  },
  {
    immediate: true,
...
watch(data, ["a", "b", "c"], (newVal, oldVal, key) => {
  console.log(
    `监听多值: ${data["a"]}-${data["b"]}-${data["c"]}, newVal = ${newVal}, oldVal = ${oldVal}, key = ${key}`
  );
});
...
watch(
  ...
  (newVal, oldVal) => {
    console.log(
      `赋值监听: z = ${data["z"]}, newVal = ${newVal}, oldVal = ${oldVal}`
    );
  },
  {
    changeTrigger: false
...
watch(
  ...
  (newVal, oldVal) => {
    console.log(
      `变化监听: z = ${data["z"]}, newVal = ${newVal}, oldVal = ${oldVal}`
    );
  },
  {
    changeTrigger: true
