  ...
  try {
    ...
    var handleExcute = (function () {
      ...
      return function (newVal, oldVal, key) {
        ...
        return !config.async
          ? _watchCxcute()
          : setTimeout(_watchCxcute, config.asyncTime || 1);
      };
    })();

    var vueWatch = null;
    watchObject &&
      (!watchObject[watchListKey] &&
        Object.defineProperty(watchObject, watchListKey, {
          configurable: false,
          enumerable: false,
          writable: false,
          value: {}
    ...
    vueWatch &&
      ...
      watchKeys.forEach(function (key) {
        ...
        if (!vueWatch[key]) {
          ...
          (keyProperty.enumerable = false), (keyProperty.configurable = false);
...
Vue.prototype.unWatch = function (watchKey) {
  try {
    var vueWatch = null;
    watchKey &&
      watchKey.watchObject &&
      (vueWatch = watchKey.watchObject[watchListKey]) &&
      watchKey.watchKeys &&
      watchKey.watchKeys.forEach(function (wKey) {
        watchKey.handleKey && vueWatch[wKey]
          ? delete vueWatch[wKey][watchKey.handleKey]
          : delete vueWatch[wKey];
      });
  } catch (error) {
    console.error(error);
  }
