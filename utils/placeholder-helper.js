...
var PlaceholderHelper = function (
  placeholderPrefix,
  placeholderSuffix,
  config
) {
  ...
  var handle = {
    ...
    replacePlaceholders: function (placeholderString) {
      ...
      var placeholderList = getPlaceholderList(
        placeholderString,
        placeholderPrefix,
        placeholderSuffix
      );
      placeholderList.forEach(function (str) {
        if (str !== placeholderSuffix) {
          ...
          // 前缀后缀不对等兼容
          if (placeholderPartStack.length !== 0) {
            var repValue = config.showPlaceholder
              ? valueList[key] !== undefined
                ? valueList[key]
 ...
 * @param {String} placeholderSuffix 后缀
 */
var getPlaceholderList = function (
  placeholderString,
  placeholderPrefix,
  placeholderSuffix
) {
  ...
  while (endIndex < placeholderString.length) {
    if (
      !placeholderList.some(function (placeholder) {
        if (
          placeholderString.substr(endIndex, placeholder.length) === placeholder
        ) {
